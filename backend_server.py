...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # Update this to the domain where your Streamlit app will run
...
async def some_whoop_data(token: str):
    logging.info(f"Received token in /some_endpoint: {token}")
    logging.info(f"Received token in /some_endpoint: {token}, Type: {type(token)}")  # Add this line
    headers = {'Authorization': f'Bearer {token}'}
    try:
        response = requests.get('https://api.prod.whoop.com/developer/v1/activity/sleep', headers=headers)
        logging.info(f"Whoop API response in /some_endpoint: {response.json()}")
        return response.json()
    except Exception as e:
        logging.error(f"Error occurred while fetching data from Whoop API: {e}")
        raise HTTPException(status_code=400, detail=str(e))
